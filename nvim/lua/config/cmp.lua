...
cmp.setup({
    ...
    sources = cmp.config.sources({
        { name = 'orgmode' }, { name = 'nvim_lsp' }, { name = 'git' }, { name = 'nvim_lua' }, { name = 'luasnip' },
    ...
    formatting = {
        format = lspkind.cmp_format({
            with_text = true,
            maxwidth = 50,
            menu = { buffer = '[buf]', calc = '[calc]', nvim_lsp = '[lsp]', nvim_lua = '[api]', path = '[path]',
                luasnip = '[snips]' }
        })
...
cmp.setup.cmdline('/',
    { completion = { autocomplete = true }, sources = { { name = 'buffer' } },
